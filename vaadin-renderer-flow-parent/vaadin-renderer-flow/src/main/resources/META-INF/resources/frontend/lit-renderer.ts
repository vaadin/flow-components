...
_window.Vaadin.setLitRenderer = (
  ...
  const renderFunction = Function(`
    ...
    return (root, {item, index, rendererOwner}, itemKey) => {
      ${clientCallables
        .map((clientCallable) => {
  ...
  const renderer: Renderer = (root, rendererOwner, { index, item }) => {
    ...
    renderFunction(root, { index, item: mappedItem, rendererOwner }, item.key);
  };
