...
public class CustomFormattingIT extends AbstractSpreadsheetIT {
    ...
    private static final String[] NAMED_COLOR_CSS = {
        "rgba(0, 0, 0, 1)",        // black
        "rgba(0, 0, 255, 1)",      // blue
    ...
    private void validateNamedColors() {
        ...
        for (int col = 0; col < COL_COUNT; ++col) {
            SheetCellElement cell = getSpreadsheet().getCellAt(ROW_OFFSET, col + COL_OFFSET);
            Assert.assertEquals(NAMED_COLOR_CSS[col], cell.getCssValue("color"));
        }
    }
    ...
    private void validateIndexedColors() {
        ...
        for (int row = 0; row < (ROW_COUNT - 1); ++row) {
            for (int col = 0; col < COL_COUNT; ++col) {
                SheetCellElement cell = getSpreadsheet().getCellAt(row + ROW_OFFSET + 1, col + COL_OFFSET);
                Assert.assertEquals(INDEXED_COLOR_CSS[row * COL_COUNT + col], cell.getCssValue("color"));
            }
        }
    }
