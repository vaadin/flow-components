...
window.Vaadin['_vaadintheme_demo_globalCss'] = window.Vaadin['_vaadintheme_demo_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_demo_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_demo_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_demo_globalCss'].push(target);
  }
  if (!document['_vaadintheme_demo_componentCss']) {
    
    document['_vaadintheme_demo_componentCss'] = true;
  }
