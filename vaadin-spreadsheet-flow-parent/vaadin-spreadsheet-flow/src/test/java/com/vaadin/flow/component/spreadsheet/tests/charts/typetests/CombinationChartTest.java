...
class CombinationChartTest extends ChartTestBase {

    @Test
    void columnAndLineTest() throws Exception {
    ...
    private void assertCombinationChartSeriesType(Configuration conf) {
        Assertions.assertEquals(5, conf.getSeries().size(),
                "Wrong series number");
    ...
    @Test
    void columnAndLineWithAxisTitlesTest() throws Exception {
        Configuration conf = getChartFromSampleFile(
                "TypeSample - Combination (Column + Line + Dual Axis).xlsx",
        ...
        Assertions.assertEquals("Title on the left",
                conf.getyAxes().getAxes().get(0).getTitle().getText());

        Assertions.assertEquals("Title on the right",
                conf.getyAxes().getAxes().get(1).getTitle().getText());
    }
}
