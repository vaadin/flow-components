 ...
 */
public class VCellNumberFormatter extends VCellFormatter {
    private static final Logger LOG = PoiLogManager.getLogger(VCellNumberFormatter.class);
    ...
    // The VCellNumberFormatter.simpleValue() method uses the SIMPLE_NUMBER
    // VCellFormatter defined here. The CellFormat.GENERAL_FORMAT CellFormat
    // no longer uses the SIMPLE_NUMBER VCellFormatter.
    // Note that the simpleValue()/SIMPLE_NUMBER VCellFormatter format
    ...
    private final VCellFormatter SIMPLE_NUMBER = new VGeneralNumberFormatter(locale);

    private static class VGeneralNumberFormatter extends VCellFormatter {
        private VGeneralNumberFormatter(Locale locale) {
        ...
        public void formatValue(StringBuffer toAppendTo, Object value) {
            ...
            if (value instanceof Number) {
                ...
                cf = (num.doubleValue() % 1.0 == 0) ? new VCellNumberFormatter(locale, "#") :
                    new VCellNumberFormatter(locale, "#.#");
            } else {
                cf = VCellTextFormatter.SIMPLE_TEXT;
            }
     ...
     */
    public VCellNumberFormatter(String format) {
     ...
     * @param format The format to parse.
     */
    public VCellNumberFormatter(Locale locale, String format) {
        super(locale, format);

        VCellNumberPartHandler ph = new VCellNumberPartHandler();
        StringBuffer descBuf = VCellFormatPart.parseFormat(format, VCellFormatType.NUMBER, ph);
    ...
    private static VCellNumberStringMod insertMod(Special special, CharSequence toAdd, int where) {
        return new VCellNumberStringMod(special, toAdd, where);
    }

    private static VCellNumberStringMod deleteMod(Special start, boolean startInclusive, Special end, boolean endInclusive) {
        return new VCellNumberStringMod(start, startInclusive, end, endInclusive);
    }

    private static VCellNumberStringMod replaceMod(Special start, boolean startInclusive, Special end, boolean endInclusive, char withChar) {
        return new VCellNumberStringMod(start, startInclusive, end, endInclusive, withChar);
    }
    ...
    public void formatValue(StringBuffer toAppendTo, Object valueObject) {
        ...
        Set<VCellNumberStringMod> mods = new TreeSet<>();
        ...
        Iterator<VCellNumberStringMod> changes = mods.iterator();
        VCellNumberStringMod nextChange = (changes.hasNext() ? changes.next() : null);
        ...
        for (Special s : specials) {
            ...
            while (nextChange != null && s == nextChange.getSpecial()) {
                ...
                case VCellNumberStringMod.AFTER:
                    ...
                    break;
                case VCellNumberStringMod.BEFORE:
                    output.insert(modPos, nextChange.getToAdd());
                    break;

                case VCellNumberStringMod.REPLACE:
    ...
    private void writeScientific(double value, StringBuffer output, Set<VCellNumberStringMod> mods) {
    ...
    private void writeFraction(double value, StringBuffer result,
            double fractional, StringBuffer output, Set<VCellNumberStringMod> mods) {
        ...
        if (!improperFraction) {
            ...
            if (fractional == 0 && !hasChar('0', numeratorSpecials)) {
                ...
                if (value == 0 && (removeBecauseZero || removeBecauseFraction)) {
                    ...
                    VCellNumberStringMod sm = hasPlaceHolder
    ...
    private void writeSingleInteger(String fmt, int num, StringBuffer output, List<Special> numSpecials, Set<VCellNumberStringMod> mods) {
    ...
    private void writeInteger(StringBuffer result, StringBuffer output,
            List<Special> numSpecials, Set<VCellNumberStringMod> mods,
        ...
        while (it.hasPrevious()) {
            ...
            if (followWithGroupingSeparator) {
                mods.add(insertMod(s, zeroStrip ? " " : groupingSeparator, VCellNumberStringMod.AFTER));
            }
            digit++;
            --pos;
        }
        if (pos >= 0) {
            ...
            mods.add(insertMod(lastOutputIntegerDigit, extraLeadingDigits, VCellNumberStringMod.BEFORE));
        }
    }
