 ...
 */
public class VCellElapsedFormatter extends VCellFormatter {
    ...
    private class VElapsedPartHandler implements VCellFormatPart.PartHandler {
        ...
        public String handlePart(Matcher m, String part, VCellFormatType type,
            ...
            case '*':
                if (part.length() > 1)
                    part = VCellFormatPart.expandChar(part);
                break;
     ...
     */
    public VCellElapsedFormatter(String pattern) {
        super(pattern);

        specs = new ArrayList<>();

        StringBuffer desc = VCellFormatPart.parseFormat(pattern,
                VCellFormatType.ELAPSED, new VElapsedPartHandler());
