...
Capabilities are delivered through `Has*` interfaces with default methods over
`getElement()`. A component implements the ones it needs; its declaration reads
...
| `HasComponents`           | `add` / `remove` / `removeAll`                      |
| `HasOrderedComponents`    | ordered add/remove/replace                          |
...
- Implement a standard interface rather than re-declaring its methods.
- A capability shared by two components goes into a shared interface in
  `vaadin-flow-components-base` — duplication is a bug, not a style choice.
