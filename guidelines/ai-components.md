...
- The only place a real LLM is used is the unpublished
  ...
  controllers server-side through `AIOrchestrator` with the Spring AI
  ...
  `AI_BENCHMARK_MODEL` is set, so it never runs in PR validation; a weekly
  TeamCity build runs it per model and graphs the pass rates. See the module
  README for configuration, output and the build recipe.
