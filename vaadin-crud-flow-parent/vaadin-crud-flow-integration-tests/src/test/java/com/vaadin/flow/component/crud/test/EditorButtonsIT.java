...
public class EditorButtonsIT extends AbstractParallelTest {
    ...
    @Before
    public void init() {
        String url = getBaseURL().replace(super.getBaseURL(), super.getBaseURL() + "/vaadin-crud") + "/editorbuttons";
        getDriver().get(url);
    }
    ...
    public void saveButtonDisabledStateCanBeControlledOnTheServer() {
        ...
        assertFalse("Save button should remain disabled", saveButton.isEnabled());
    ...
    public void cancelButtonDisabledStateCanBeControlledOnTheServer() {
        ...
        assertFalse("Cancel button should be disabled", cancelButton.isEnabled());

        TextFieldElement lastNameField = crud.getEditor().$(TextFieldElement.class)
                .attribute("editor-role", "last-name").first();
        lastNameField.setValue("Otto");

        assertFalse("Cancel button should remain disabled", cancelButton.isEnabled());
    ...
    public void deleteButtonDisabledStateCanBeControlledOnTheServer() {
        ...
        assertFalse("Delete button should be disabled", deleteButton.isEnabled());

        TextFieldElement lastNameField = crud.getEditor().$(TextFieldElement.class)
                .attribute("editor-role", "last-name").first();
        lastNameField.setValue("Otto");

        assertFalse("Delete button should remain disabled", deleteButton.isEnabled());
    ...
    public void buttonsCanWorkWithShortcutsDefinedOnServer() {
       ...
       String LAST_NAME_ACTUAL = crud.getGrid().getCell(0, 2).getText();
        ...
        assertEquals("Last name is updated", LAST_NAME_EXPECTED, LAST_NAME_ACTUAL);
    }
