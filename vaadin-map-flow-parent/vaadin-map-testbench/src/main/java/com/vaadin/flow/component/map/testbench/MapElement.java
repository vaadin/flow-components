...
public class MapElement extends TestBenchElement {
    ...
    private void ensureMapReady() {
        if (!mapReady) {
            // Flush pending Vaadin server state so the connector synchronize()
            // has run and the OpenLayers view has center/zoom set.
            getCommandExecutor().waitForVaadin();
            // Wait for a render that produced a non-null frameState.
            // postrender fires even when frameState is null (view has no
