...
public abstract class AbstractCompatChartsIT extends AbstractParallelTest {
    ...
    public void testScreenshot() throws Exception {
        open(getViewClass());
        String chartType = getViewClass().getSimpleName();
        compareScreen("initial" + chartType);

        $(ButtonElement.class).id(AbstractComponentTestView.MIDDLE_SIZE_BTN)
                .click();
        compareScreen("middle" + chartType);

        $(ButtonElement.class).id(AbstractComponentTestView.SMALL_SIZE_BTN)
                .click();
