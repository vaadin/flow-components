...
@Route("vaadin-list-box/selection-preservation")
public class SelectionPreservationPage extends Div {
    ...
    public SelectionPreservationPage() {
        ...
        var listBox = new MultiSelectListBox<Integer>();
        listBox.setItems(dataProvider);
        listBox.setItemLabelGenerator(String::valueOf);

        var modePreserveAll = new NativeButton("Preserve All",
                e -> listBox.setSelectionPreservationMode(
        ...
        var showServerValue = new NativeButton("Show server value",
                e -> serverValue.setText(listBox.getSelectedItems().stream()
        ...
        add(listBox, modeButtons, actionButtons, serverValue);
    }
}
