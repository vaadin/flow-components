...
import type { ComboBoxConnector, FlowComboBox, Item, ItemRange } from './vaadin-combo-box-types.js';
...
function initLazy(comboBox: FlowComboBox): void {
  ...
  let cache: Record<number, Item[]> = {};
  const placeHolder = new window.Vaadin.ComboBoxPlaceholder();

  let lastTypedFilter = '';
  let lastRequestedRange: ItemRange = [-1, -1];
  let lastRequestedFilter = '';
  let needsDataCommunicatorReset = false;

  const dataProvider: ComboBoxDataProvider<Item> = function (params, callback) {
  ...
  const performClientSideFilter = function (
    page: Item[],
    filter: string,
    callback: ComboBoxDataProviderCallback<Item>
  ): void {
  ...
  const $connector: ComboBoxConnector = {
    getViewportRange() {
    ...
    requestPage(page, filter) {
      const viewportRange = comboBox.$connector.getViewportRange();
    ...
    clear(start, length) {
      ...
      for (let index = firstPage * pageSize; index < lastPage * pageSize; index++) {
        if (filteredItems[index]) {
          ...
          filteredItems[index] = placeHolder as Item;
        }
      }
    },
