...
public class MultiSelectComboBoxUserValueChangeIT extends AbstractComponentIT {
    ...
    @Test
    public void selectItemFromDropdown_valuePropagatedToServer() {
        clickItem("Item 1");
    ...
    @Test
    public void deselectItemFromDropdown_valuePropagatedToServer() {
        clickItem("Item 1");
        waitUntil(driver -> "Item 1".equals(eventValue.getText()));
    ...
    public void removeChip_valuePropagatedToServer() {
        ...
        // The first chip is the overflow chip, so the actual item chip is the
        // second one.
    ...
    @Test
    @Ignore("Depends on web-components PR 11988 (Esc-clear dispatching `change`)"
            + " — enable after the @vaadin/multi-select-combo-box version is bumped")
    public void pressEscapeToClear_valuePropagatedToServer() {
        clickItem("Item 1");
        waitUntil(driver -> "Item 1".equals(eventValue.getText()));

        // Clear button must be visible and the overlay closed for Esc to clear.
